...
def CAccuracy(output, target):
    acc = [0 for i in range(output.size()[1])]
    with torch.no_grad():
        _, pred_k = output.max(dim=-1)
        for i in range(output.size()[1]):
            idx = (target == i)
            acc[i] = ((pred_k[idx]==i).sum()).item()
    return acc
...
class Metric(object):
    def __init__(self, metric_name):
        ...
        self.metric = {'Loss': (lambda input, output: output['loss'].item()),
                       ...
                       'CAccuracy': (lambda input, output: recur(CAccuracy, output['target'], input['target']))}

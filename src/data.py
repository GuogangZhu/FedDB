...
def fetch_dataset(data_name):
    ...
    if data_name=='CIFAR10':
        dataset['train'] = datasets.CIFAR10(root=root, split='train', transform=datasets.Compose([transforms.ToTensor()]))
        dataset['test'] = datasets.CIFAR10(root=root, split='test', transform=datasets.Compose([transforms.ToTensor()]))
        ...
        dataset['train'] = datasets.CIFAR100(root=root, split='train', transform=datasets.Compose([transforms.ToTensor()]))
        dataset['test'] = datasets.CIFAR100(root=root, split='test', transform=datasets.Compose([transforms.ToTensor()]))
        dataset['train'].transform = datasets.Compose([
            transforms.RandomHorizontalFlip(),
            transforms.RandomCrop(32, padding=4, padding_mode='reflect'),
            transforms.ToTensor(),
...
def split_dataset(dataset, num_users, data_split_mode):
    ...
    if data_split_mode == 'iid':
        ...
        data_split['train'] = non_iid(dataset['train'], num_users, data_split_mode)
        data_split['test'] = non_iid(dataset['test'], num_users, data_split_mode) # 测试集为什么也要划分。。。后面还会用到吗
    else:
...
def non_iid(dataset, num_users, data_split_mode):
    ...
    data_split_mode_list = data_split_mode.split('-')
    ...
    # direclet distribution
        ...
        while min_size < required_min_size :
            ...
            for target_i in range(cfg['target_size']):
                ...
                split_idx = np.array_split(target_idx, split_idx) # 平台上的版本太旧，将torch.tensor_split换成np.array_split
...
def separate_dataset_su(labeled_dataset, unlabeled_dataset=None, supervised_idx=None):
    # sample labeled and unlabeled dataset, and distribute them to each client
    target = torch.tensor(labeled_dataset.target)
    ...
    idx = list(range(len(labeled_dataset)))
    unsupervised_idx = list(set(idx) - set(supervised_idx))
    _labeled_dataset = separate_dataset(labeled_dataset, supervised_idx)
    if unlabeled_dataset is None:
        _unlabeled_dataset = separate_dataset(labeled_dataset, unsupervised_idx)
    else:
        _unlabeled_dataset = separate_dataset(unlabeled_dataset, unsupervised_idx)
        transform = FixTransform(cfg['data_name'])
        _unlabeled_dataset.transform = transform
    return _labeled_dataset, _unlabeled_dataset, supervised_idx

def make_batchnorm_dataset_su(labeled_dataset, unlabeled_dataset):
    batchnorm_dataset = copy.deepcopy(labeled_dataset)
    batchnorm_dataset.data = batchnorm_dataset.data + unlabeled_dataset.data
    batchnorm_dataset.target = batchnorm_dataset.target + unlabeled_dataset.target
    batchnorm_dataset.other['id'] = batchnorm_dataset.other['id'] + unlabeled_dataset.other['id']
